...
- Sauveteur en mer, SNSM, depuis septembre 2022. TROIS saisons de 2 mois à Saint-Malo, la 3e étant EN COURS (soit 6 mois cumulés) — et non trois saisons terminées plus une quatrième en cours. Détail par saison : 1 saison comme sauveteur qualifié, 2 saisons comme adjoint chef de poste secondant un chef de poste, et la saison en cours comme adjoint chef de poste sans chef de poste — Côme gère son poste de secours seul, fonction de chef de poste exercée, grade non accordé par la mairie. Sept diplômes : PSE1, PSE2, BNSSA, SSA mention littoral, permis bateau, CRR, MJ1. Postes de secours (DPS) sur concerts, festivals, matchs du Stade Rennais.
  NOTE DE FORMULATION (29/07/2026) : la rédaction précédente ("Trois saisons […] Saison en cours : …") était ambiguë et a conduit une session Claude à DÉDUIRE une 4e saison inexistante, puis à déclarer le point tranché. Toute session future qui a besoin d'un décompte de saisons doit DEMANDER à Côme, jamais déduire d'un doc. Rappel de la règle d'ouverture de cette section : ne rien déduire sur Côme ; si l'information n'est pas ici, la demander.
...
- **Dans l'éditeur SQL de Supabase, un résultat collé ne prouve pas qu'une requête a été
  exécutée.** L'état se contrôle dans un onglet séparé, par une requête dont le résultat
  diffère selon que l'opération a eu lieu ou non. Origine : session du 16/09/2026, verrou
  d'écriture de `users`.
...
Claude Code replie les sorties longues et n'en montre qu'une partie. Six comportements, observés les 15 et 16/09/2026, s'ajoutent à ce repli. (1) Une liste de sept lignes a été repliée après sa troisième ligne : les listes se lisent par tranches de trois lignes, toute mention « +N lines » invalide la lecture, et une liste qui décide d'un push se relit dans le Terminal macOS. (2) Cinq fois dans la même session, Claude Code a remplacé la commande demandée par son propre outil de recherche ou de lecture, dont le résultat est replié, puis a écrit lui-même le nombre ou la position attendus ; quatre de ces cinq fois malgré la consigne explicite « n'utilise que l'outil Bash ». La consigne ne suffit donc pas à l'empêcher. Un nombre rendu ainsi n'est pas une sortie brute : il se refuse et se contrôle dans le Terminal macOS. (3) Le dossier courant d'une session Claude Code persiste d'un prompt à l'autre : la consigne « place-toi à la racine » ne suffit pas, chaque commande qui dépend du dossier commence par `cd /Users/comefourel/Dev/sterny &&`. (4) Une sortie ne se relit pas après coup : quand elle doit être lue par tranches, la commande l'écrit d'abord dans un journal, qu'on lit ensuite, et une commande qui agit (un push, une écriture) ne se relance jamais pour être relue.
