...
**Accès à la table `users` (décision du 16/09/2026, révisée le 21/09/2026, DETTE #171).** Aucune donnée de `users` n'est lisible sans connexion. Un utilisateur connecté lit sa propre ligne en entier ; l'admin lit tout. Il lit la ligne entière d'un autre compte seulement dans cinq cas : (a) ils sont les deux parties d'un même contrat ou d'un même renouvellement ; (b) l'autre compte a candidaté à l'une de ses annonces, quel que soit le statut de la candidature ; (c) il a candidaté à une annonce de l'autre compte et la candidature est acceptée ; (d) l'un est le parrain de l'autre, dans un sens ou dans l'autre ; (e) l'autre compte a une candidature acceptée sur une annonce dont l'auteur est lié au lecteur par parrainage. Les messages, les mises en relation et les avis n'ouvrent aucune lecture de la ligne : ces surfaces passent par le profil public. Ce profil public est servi par une fonction de la base, interrogeable pour plusieurs comptes à la fois et accessible sans connexion : prénom, nom, photo, type, école, année, filière, bio, villes et statut « identité vérifiée » ; jamais le téléphone, l'email, la date de naissance ni le rythme. Une seconde fonction, accessible sans connexion, rend l'identifiant, le prénom et le nom du parrain à partir d'un jeton d'invitation. Le parrain d'un compte n'est jamais écrit par le navigateur : il est renseigné par la base, soit à la création du compte, soit, pour une inscription Google ou Apple, par une fonction dédiée qui n'agit que si aucun parrain n'est encore enregistré. Lors d'une inscription par email des parcours propriétaire et « Proposer un logement », la ligne `users` est créée par la base à la naissance du compte : le type est déduit du parcours, l'email est pris dans le compte d'authentification, les autres champs sont limités à une liste fermée (prénom, nom, téléphone, rythme, ville). Si cette création échoue, toute l'inscription échoue. La règle d'insertion ouverte est supprimée et la vérification d'existence d'un email est abandonnée. Limite assumée : tant que les candidatures, les contrats, les annonces et les renouvellements restent écrits sans contrôle par le navigateur, les cas (b), (c) et (e), ainsi que le cas (a) pour les renouvellements, sont falsifiables ; leur fermeture (phase A bis, DETTE #177) précède toute ouverture de la plateforme. Dans un second temps, les champs visibles entre comptes en relation seront restreints, relation par relation (phase B, DETTE #175).
