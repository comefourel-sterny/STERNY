...
**Dernière mise à jour** : 2026-09-16 — Accès à la table `users` : lecture sans connexion fermée, lecture limitée aux comptes en relation, ligne créée par la base à l'inscription.
...
**Accès à la table `users` (décision du 16/09/2026, DETTE #171).** Aucune donnée de `users` n'est lisible sans connexion. Un utilisateur connecté lit sa propre ligne en entier, et celle d'un autre compte uniquement s'il existe entre eux une relation réelle : candidature, contrat, message, parrainage ou mise en relation validée. L'admin lit tout. Un compte sans relation n'est visible qu'à travers un profil public : prénom, nom, photo, type, école, année, filière, bio, villes et statut « identité vérifiée ». Le téléphone, l'email, la date de naissance et le rythme n'y figurent jamais. Lors d'une inscription par email, la ligne `users` est créée par la base à la naissance du compte, à partir d'une liste fermée de champs ; le parrain est retrouvé par la base depuis le jeton d'invitation, jamais transmis par le navigateur. La vérification d'existence d'un email est abandonnée. Dans un second temps, les champs visibles entre comptes en relation seront restreints, relation par relation.
