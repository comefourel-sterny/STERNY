...
5. **useEffect en boucle** (ligne ~562-579) : re-render 5-6× à chaque changement de state. Le useEffect modifie ses propres dépendances via `processRhythmDates`. À refactoriser avec un `useRef` ou une condition de sortie.
   - MAJ conv 91 (25 juin 2026) : ✅ RÉSOLUE — la boucle venait du useEffect « Auto-enter cycle selection » (le log [Calendar debug] qui se redéclenchait), retiré avec le moteur de cycle (grappe 4). Vérifié runtime : filtre console « Calendar debug » = 0 résultat, planche affichée, plus de cascade de re-render auto.
...
**MAJ 20/08/2026 — un troisième état apparaît sur RhythmCalendar.** Le composant affichait deux états, école et entreprise. L'alignement sur la géométrie de la planche introduit un état neutre pour les semaines non renseignées, choix de code couleur pris explicitement et non subi. Le point 3 de cette dette (navy rendu de trois façons) gagne une conséquence directe, décrite dans la dette ouverte le même jour.
...
MAJ conv 91 (suite, 25 juin 2026) : grappes 3bis (encadré récap « plage continue » DÉBUT→FIN + ligne « Période » de la modale, ~52 l.) et 4 (moteur de cycle complet : 9 fonctions + 2 useEffect dont la boucle DETTE #5 — RÉSOLUE — + ~178 l.) sorties, build vert, never-stage. MAPPING wizard confirmé en lecture (l.1576) : currentStep 0=Bail · 1=Informations · 2=Détails · 3=Photos · 4=Disponibilités(planche) · 5=Prix — d'où le décalage code/écran (« step 4 » code = « étape 5 Disponibilités » écran). Le nettoyage planche/disponibilités (currentStep 4) est TERMINÉ côté code mort. RESTENT, HORS périmètre planche, en grappe « étape 0 Bail » dédiée : (a) 8 states write-only (bailStartDate/bailEndDate/bailDuree/rhythmStartDate/rhythmEndDate/rhythmType/rhythmPattern/dimancheChoixFaitRef) — écrits par l'extraction PDF bail + removeBailFile + prefill, jamais relus ; (b) ⚠️ prefill l.640-648 lit type_alternance/rythme_alternance (colonnes DÉPRÉCIÉES gelées → viole invariant 5), mort en pratique, à retirer ; (c) la feature import PDF bail (étape 0) alimente aussi pricing/dpe/propertyInfo (lus) → décision produit + audit avant tout retrait. La ZONE D'IMPORT étape 0 Bail est à PRÉSERVER (consigne Côme conv 91 : elle sert au fonctionnement et à la vérification des annonces). Reste aussi : « Réinitialiser » (ex-Tout effacer, clearAllDates conservée) + passe finale (header « Disponibilités & Bail » périmé, imports, CSS orphelines, joursNoms, logs DEV) puis ModifierAnnoncePage (Lot 6).
...
## DETTE #120 — Sauvegarde de ModifierAnnoncePage gèle (étape Prix) — RÉSOLU côté page (cause racine useAuth ouverte en #121)
**Statut : RÉSOLU côté ModifierAnnoncePage (conv 97, 29 juin 2026), fix validé runtime. Cause racine globale (useAuth) ouverte séparément en #121.**
CAUSE RACINE PROUVÉE : boucle de re-rendu / requêtes infinie. Le useEffect de chargement de ModifierAnnoncePage dépendait de l'OBJET `user` (deps [user], vers l.407). Or useAuth fait `setUser(session.user)` à chaque événement d'authentification : `session.user` est une NOUVELLE référence à chaque fois, MÊME pour le même utilisateur (id stable). Avec autoRefreshToken activé (défaut Supabase), chaque requête de la boucle déclenchait un refresh de token → événement auth → nouvelle référence `user` → l'effet [user] se relançait → nouvelle requête `users` → re-refresh… boucle auto-entretenue. Mesuré runtime : 4000+ requêtes `users`/`token?grant_type=refresh_token` sans interaction, page saturée (gel, scroll bloqué via overflow:hidden de la modale jamais peinte, save inopérant car React n'avait plus de cycle libre).
DIAGNOSTIC (conv 96-97) : mené par mesures runtime successives (Network filtré token/users vidé + Preserve log décoché ; Console filtre -4747 ; comptage [6a]). PIÈGE rencontré : le compteur Network cumulé (Preserve log) faisait croire que la boucle persistait après le fix → la mesure fiable est le DÉBIT sur fenêtre vidée, pas le total. Autre piège : le HMR de Vite ne réapplique pas proprement un changement de DEPS de useEffect → redémarrage complet de `npm run dev` nécessaire pour tester le fix (sinon ancien bundle).
FIX APPLIQUÉ : dépendance de l'effet passée de [user] à [user?.id] (chaîne stable). Validé sûr par audit lecture seule : dans cette page `user` n'est lu que via sa véracité (if (!user)) et `user.id` (2 sites) ; jamais un autre champ, jamais l'objet entier transmis. Validé runtime : console vide ([6a] = 0 sur fenêtre propre), plus de rafale token, page fluide.
PÉRIMÈTRE : le fix ne corrige QUE ModifierAnnoncePage. La racine (useAuth recrée `user` à chaque event) reste latente pour tout autre useEffect([user]) de l'app → DETTE #121.
CONFIRMATION (conv 99) : le doute conv 98 ("fix [user?.id] insuffisant, ca boucle encore") venait d'un bundle Vite perime, pas du fix. Sur bundle frais, chargement calme. Fix #120 CONFIRME. Le vrai symptome restant etait au CLIC (modale masquee) = #123, sans rapport avec cet effet.
CORPS PÉRIMÉ SUPPRIMÉ : l'ancienne rédaction « test causal n'a pas pu être mené, pannes réseau » était antérieure au test (mené en conv 95, coupes 6c innocentées). Corrigée.

## DETTE #121 — useAuth : `user` recréé à chaque événement auth (fragilité globale de re-rendu)
**Statut : OUVERTE (constat conv 97, 29 juin 2026). Racine du bug #120. Touche toute l'app → session dédiée, prudence (fichier central).**
CONSTAT : useAuth fait `setUser(session?.user ?? null)` sur getSession initial ET sur chaque onAuthStateChange (refresh token, etc.). `session.user` est une nouvelle référence à chaque fois, même pour le même utilisateur. De plus, la value du context (`value={{ user, session, loading, signOut }}`) est un objet littéral RECRÉÉ à chaque rendu (pas de useMemo). CONSÉQUENCE : tout consommateur avec un useEffect([user]) (ou dépendant de la value entière) se relance à chaque événement auth → risque de boucle/sur-rendu identique à #120, partout dans l'app.
FIX LOCAL DÉJÀ POSÉ (#120) : ModifierAnnoncePage utilise [user?.id]. Mais c'est un contournement page par page.
...
**Statut : RESOLU (conv 99, 30 juin 2026), fix commite a3c1761 + valide runtime. Cause reelle = collision de classe CSS globale, liee a #86.**
CAUSE REELLE PROUVEE : la modale de confirmation du save utilisait la classe GLOBALE .modal-overlay. Une regle globale concurrente (.modal-overlay { display:none } dans ContratLocationPage.css, non scopee — DETTE #86) la masquait : la modale se rendait bien dans le DOM (showConfirmModal=true, div present) mais restait INVISIBLE (display:none), tandis que document.body.style.overflow='hidden' bloquait le scroll → symptome "page figee, rien ne se passe AU CLIC sur Enregistrer". Ce n'etait JAMAIS une boucle de re-rendu ni un thread bloque.
PIEGE DE DIAGNOSTIC LEVE : le "gel AU CHARGEMENT" suspecte en conv 98 (logs [6a] en rafale, fix #120 "insuffisant") etait un ARTEFACT DE BUNDLE VITE PERIME. Le HMR de Vite ne reapplique pas un changement de deps de useEffect (deja note en #120) : la mesure "ca boucle encore" tournait sur l'ancien bundle. Sur bundle frais (Ctrl+C + npm run dev + Cmd+Shift+R), le chargement est calme, [6a] apparait 2 fois (StrictMode dev) puis s'arrete. Le fix #120 ([user?.id]) est CONFIRME suffisant.
FIX : renommage de la classe en .ma-confirm-overlay (classe scopee unique, .jsx + .css), meme remede que InvitationModal (.inv-overlay). Audit lecture seule complet (effets, deps, checkUserAndLoadAnnonce, chemin save, CSS) avant tout patch — aucun moteur de boucle interne au fichier, ce qui a oriente vers la cause CSS.
VALIDE RUNTIME : clic Enregistrer → modale affichee centree avec recap, console vide ; chemin de save execute jusqu'a la garde identite (compte de test identite_verifiee != 'verifiee' → message Stripe Identity attendu, NORMAL).
[PISTE ABANDONNEE conv 98 — fausse, voir statut RESOLU ci-dessus] SYMPTÔME : au CHARGEMENT de ModifierAnnoncePage (avant tout clic), la page se fige ; la modale de confirmation du save n'est jamais peinte. Console : "Navigated to .../annonce/modifier" + "[6a] hostProfile chargé" en rafale.
[PISTE ABANDONNEE conv 98 — fausse, voir statut RESOLU ci-dessus] ACQUIS (test runtime conv 98) : boucle SPÉCIFIQUE à cette page (/recherche + home = calmes) → PAS un composant global (NE PAS re-suspecter PasswordGate/useAuth/routeur). Les logs ModifierAnnoncePage.jsx continuent APRÈS navigation ailleurs → le composant ne se démonte pas, un useEffect boucle sans nettoyage. AUCUN window.location/reload dans le chemin de chargement (écarté). Le fix #120 ([user?.id]) n'a PAS suffi.
[PISTE ABANDONNEE conv 98 — fausse, voir statut RESOLU ci-dessus] HYPOTHÈSE : dépendance OBJET/TABLEAU recréée à chaque rendu (manifestation locale de #121). SUSPECTS : useEffect l.~297 (deps [hostProfile, natureLogement, semainesLibres]) et useEffect l.~404 (deps [user?.id]). Re-localiser par grep (numéros décalés depuis retrait agentation).
...
**Statut : TRANCHÉE intellectuellement (conv 101) — N'EST PLUS un blocage contractuel. Décision produit prise ; mise en œuvre SUSPENDUE à la reprise de 6c et aux réponses pro sur le bail. Plus un bloqueur du sous-pas 3 au sens « contractuel », mais le sous-pas 3 reste gelé pour une raison de SÉQUENCEMENT (dépend du chantier stockage/preuve du bail).**
...
RÉSOLUTION (conv 101, audit lecture seule + décision produit) : verdict factuel = bail_info porte les dates du BAIL PRINCIPAL de l'hôte, PAS le contrat de sous-location (qui vit dans `contrats`, figé à la signature → c'est lui que vise §145). Donc l'écart Creer(null)/Modifier(vraies dates) n'a JAMAIS été un risque contractuel/eIDAS. Décision produit : GARDER les dates + CONSERVER le document de bail (preuve anti-falsification) + ALIGNER Creer sur Modifier (et non l'inverse). Mise en œuvre repoussée : elle est couplée au nouveau chantier « stockage + preuve d'intégrité du bail » à cadrer avec agent immobilier + avocate + Pépite avant toute implémentation (preuve d'intégrité = architecture, à penser tôt ; RGPD = pré-lancement, repoussable car prod à ~1 an). Reliquat de vérification : grep sur supabase/migrations pour confirmer qu'aucune vue SQL ne lit bail_info (non couvert par l'audit conv 101).
...
ModifierProfilPage.jsx stocke "Fougeres"/"Vitre" (sans accent) alors que CreerAnnoncePage.jsx stocke "Fougères"/"Vitré" (avec accent) pour les mêmes villes. Toute comparaison stricte de ville échoue silencieusement pour ces 2 villes selon la page utilisée pour la saisie. Découvert le 12/07/2026 en auditant le matching ville visiteur/annonce. Contourné localement dans LogementPage.jsx (comparaison normalisée accents/casse via normalizeVilleLabel). Non corrigé à la source : nécessite une liste de villes unique réutilisée partout, à traiter en session dédiée.

## DETTE #137 — Fuite CSS globale .section-title depuis CreerAnnoncePage.css

CreerAnnoncePage.css définit un .section-title non scopé avec !important sur toutes les propriétés (15px/300/orange/uppercase/letter-spacing 2px). Le CSS du projet étant global (pas de CSS modules), cette règle écrase .section-title sur toute la plateforme, y compris LogementPage.css (20px/700/navy) et ModifierAnnoncePage.css (24px/700/navy) — !important bat la spécificité et l'ordre de chargement normaux. Découvert le 13/07/2026. Fichier concerné (CreerAnnoncePage.css) actuellement never-stage — correction impossible avant Phase 0bis. À traiter : scoper le .section-title de CreerAnnoncePage.css (ex. .creer-annonce-container .section-title) comme le font déjà EtatDesLieux et Dashboard.

## DETTE #138 — Carte Disponibilités : exclusion des semaines déjà logées ailleurs, branchée mais no-op tant que le registre n'est pas alimenté

semainesBesoinAnnonce (LogementPage.jsx, useEffect "1a") exclut désormais les semaines présentes dans semaines_reservees pour le visiteur connecté (requête WHERE locataire_id = user.id, en parallèle de la requête users via Promise.all). Comme cette table est vide et sans policy RLS (Tranche B gelée, DETTE #93, "gel aval conv 103"), la requête renvoie toujours une liste vide aujourd'hui — le filtre est un no-op silencieux, sans effet visible tant que le registre n'est pas branché. Dès que la Tranche B connecte le registre et ajoute une policy RLS de lecture, ce filtre s'activera automatiquement sans modification de code. Implémenté le 13/07/2026 en lecture pure (aucune écriture, aucun câblage du verrou signature) — ne touche pas le côté "write" gelé du #93. À vérifier avec les pros lors des RDV : la policy RLS à ajouter (lecture des semaines réservées par le locataire concerné uniquement — implique de valider avec le juriste/DPO qui peut lire quoi sur semaines_reservees).
...
**Trois tentatives écartées, avec leur motif** : gris #F4F5F7 plein, indiscernable ; blanc à contour, faux au sens de la grammaire de la planche où ce rendu signifie « à couvrir », et rendant le vide plus visible que la donnée ; gris atténué et flouté de l'état hors-sujet, qui floute aussi le numéro de jour et se lit comme un défaut d'affichage.
**Ce qu'il faut trancher** : la valeur d'opacité de l'entreprise. L'en-tête de `RhythmCalendar.css` interdit de descendre sous 15 %, règle écrite quand le composant n'avait que deux états et que le contraste ne se jouait que contre l'orange. Un troisième état change la donne. Passer au navy plein du builder est la piste, elle n'est pas décidée.
...
**MISE À JOUR 2026-09-22, PRODUCTION. PHASE A CLOSE.** Migration appliquée seule dans l'éditeur SQL de production, sans déploiement : le site sert `main` (56289dc), dont le public ne voit que la landing, qui ne touche pas `users`. Contrôle avant et après, conforme au local : 7 empreintes, 6 règles, sécurité par ligne active, déclencheur sur `auth.users`, droits d'appel exacts. La lecture sans connexion est fermée en production. Build de l'état commité réussi, push fait (c9e3b92). Restent ouvertes : les relations falsifiables (phase A bis, DETTE #177) et la restriction des champs (phase B, DETTE #175). L'ancien code de `main` est cassé derrière le mot de passe (DETTE #186).
**Statut** : phase A close le 2026-09-22. Dette ouverte jusqu'aux phases A bis et B.
...
**CLOSE 2026-09-23.** Migration `20260923090000_creer_profil_sans_partager.sql` (805d3c3) appliquée en local puis en production : le déclencheur n'accepte plus que le parcours propriétaire et n'écrit plus `rythme_alternance` ni `a_logement`. Empreintes identiques sur les deux bases (37ab0e06…1a7886). Les autres lectures et écritures de `rythme_alternance` dans `sterny-react` (CompleterProfilPage, InscriptionRecherchePage, ModifierProfilPage, DashboardLocatairePage, DashboardProprietairePage, ProfilPage, CreerAnnoncePage) relèvent de l'invariant 5 et restent hors de cette dette.
...
**Découverte** : 2026-09-22.
**MISE À JOUR 2026-09-22, PRODUCTION.** Registre de production relevé : dernière entrée `20260425121949`. Les migrations appliquées depuis par l'éditeur SQL, dont `20260915120000`, `20260916090000` et `20260922090000`, n'y figurent pas. Les deux registres divergent ; même règle : aucune commande `db reset` ni `db push`.
**MISE À JOUR 2026-09-23.** `20260923090000` appliquée par psql en local et par l'éditeur SQL en production, absente des deux registres comme les précédentes. Même règle.

## DETTE #186 — `main` en production : ancien code cassé derrière le mot de passe
**Constat (phase A de la DETTE #171, 22/09/2026)** : la production sert `main` (56289dc, 22/06), qui n'a pas les sept pages de la phase A. Depuis la migration du 22/09, derrière le mot de passe de `PasswordGate`, l'ancien code échoue sur l'invitation et l'inscription propriétaire (lecture de `invitation_token`), sur les inscriptions recherche, partager et propriétaire (insertion de la ligne `users` par le navigateur) et sur le parrainage de la connexion Google (lecture d'un autre compte, écriture de `parrain_id`) ; il n'affiche plus la carte de l'hôte, les avis, les noms dans la messagerie ni la recherche par email du dashboard locataire. La landing publique n'est pas touchée. Par ailleurs, `PasswordGate` vérifie le mot de passe dans le navigateur, contre une empreinte écrite dans le code : ce n'est pas une protection.
