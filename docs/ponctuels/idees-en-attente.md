...
- 2026-08-03 — MÉNAGE DISQUE, session dédiée. État vérifié ce jour, remonté ici parce qu'il partait en archive avec la coupe d'ETAT-COURANT. FAIT : le backup `sterny-backup-before-filter-repo-20260423-2326` a bien été supprimé. RESTE : cinq copies du projet subsistent dans `~/Dev/` et ailleurs, toutes figées au 13 avril 2026, environ 1,2 Go au total — `sterny-old` (596 M), `sterny-backup-avant-git` (587 M), `sterny-come-local-13avril-23h` (4,3 M), `~/Desktop-backup-sterny-20260413` (4,3 M), et une copie iCloud dans `CloudDocs/Desktop/STERNY` (292 K). RESTE AUSSI : la synchro iCloud du Bureau est toujours ACTIVE, c'est la cause probable de la copie iCloud, à désactiver via Réglages Système → identifiant Apple → iCloud → iCloud Drive → Options → décocher Dossiers Bureau et Documents. MÉTHODE OBLIGATOIRE avant toute suppression : vérifier que chaque copie ne contient rien qui n'existe pas déjà dans le dépôt git actuel. Aucune n'a bougé depuis le 13 avril, donc aucune ne porte de travail récent, mais une seule vérification ratée est irréversible. Ne pas traiter en marge d'un autre chantier.
...
**V2 questionnaire terrain — implémentée et auditée (25/07/2026)** : ajout de la question échelle de difficulté logement (section "Difficultés logement", 4 niveaux, texte libre facultatif affiché pour tout sauf "Aucune difficulté") + question "Pourquoi ?" conditionnelle (affichée pour toute réponse sauf "Je ne sais pas" sur la question hypothétique existante). Un bug d'orphelinage a été découvert et corrigé le 25/07 : la section "Difficultés logement" n'était reliée à aucun chemin du formulaire — tous les parcours (trajets, logement payé, deux logements, location courte durée, chemins famille/sans famille y compris les options "Autre") sautaient directement vers "Choix pro". Les 11 points de redirection concernés ont été audités un par un et recâblés pour passer systématiquement par "Difficultés logement" avant "Choix pro", sans toucher au reste du parcours existant. Formulaire toujours PUBLIÉ, pas de nouveau gate RGPD requis (question dans la continuité du périmètre déjà validé par Benoît). Tests de parcours complets effectués le 25/07 sur 9 profils couvrant l'intégralité des branches (rythme, mono/bi-ville, famille/sans famille, tous modes de logement, renoncement à une opportunité) — aucune anomalie de câblage détectée. Deux points de design confirmés intentionnels et laissés tels quels : les questions "as-tu rencontré des difficultés" et "si le logement n'avait pas été un problème" restent universelles (posées à tous, y compris "aucune difficulté"/"non jamais"), car elles capturent un signal différent (vécu concret vs. influence non-consciente sur les choix).
...
**Audit du 09/09/2026, en lecture seule.** Le stockage, la réédition et la recherche sont déjà découplés du rythme et accepteraient une semaine de vacances sans modification. Le seul verrou est l'ensemble de semaines fourni à la planche par la page de création. Aucun élargissement de cet ensemble n'est honnête aujourd'hui : les semaines absentes n'existent pas à l'intérieur d'une année déclarée, et l'année complète autoriserait une offre contredisant le rythme. La résolution passe par un troisième statut dans `rhythm_calendar`, chantier à cadrer pour lui-même. Détail complet en ETAT-COURANT, entrée du 09/09.
