...
**Dernière mise à jour** : 2026-06-24 (conv 84 suite) — Audit CreerAnnoncePage : page pré-calendrier, ne produit PAS un disponibilites_pattern conforme VISION (DETTE #113). Plan : annonce de test via SEED maîtrisé (pas via la page), finir /logement 1a→1b AVANT de refondre CreerAnnoncePage.
...
RESTE (non bloquant, prochaine session) : créer un compte HÔTE avec rythme (pour avoir une annonce à matcher), reprendre 1a→1b sur /logement sur données réelles, puis committer l'étape 1a restée dans le working tree depuis conv 83.
