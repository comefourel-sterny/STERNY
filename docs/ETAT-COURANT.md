...
**Dernière mise à jour** : 2026-06-24 (conv 88) — Refonte CreerAnnoncePage : audit Lot 3 complet (3 passes), 3 décisions figées (ville←profil, bail supprimé, découpage 3a/3b/3c). Aucun code touché. Cap : exécuter 3a→3c en conv fraîche.
...
DÉCISIONS FIGÉES :
- Ville→nature = depuis le profil (VISION §652 réaffirmée). annonces.ville ← deduireOffre(hostProfile) (ville action='hote'), plus villeDetectee (table CP en dur). Cas courant 1 ville hote → ville FIXÉE lecture seule, AUCUN sélecteur. CP/adresse conservés UNIQUEMENT pour le GPS (verifiedCoordinates, api-adresse). les_deux 2 villes hote (§91/§653) = neutralisé défensivement, différé.
